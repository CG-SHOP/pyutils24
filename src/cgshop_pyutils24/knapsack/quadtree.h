...
namespace cgshop2024
{
        ...
        public:
            ...
            Rectangle(Num x, Num y, Num width, Num height) : x(x), y(y), width(width), height(height)
            {
                if (width <= 0 || height <= 0)
                {
                    // Just to make sure that we didn't do something stupid.
                    throw std::runtime_error("Rectangle must have positive width and height.");
                }
            }
            ...
            std::array<Rectangle, 4> split() const
            {
                ...
                assert(result[0].volume() + result[1].volume() + result[2].volume() + result[3].volume() == volume());
                return result;
            }
                ...
                else
                {
                    ...
                    for (auto &element : elements)
                    {
                        if (element.area.do_overlap(query_area))
                        {
                            ...
                            if (std::find(result.begin(), result.end(), element.data) == result.end())
                            {
